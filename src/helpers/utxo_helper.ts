...
export async function avmGetAllUTXOs(addrs: string[], sourceChain?: string): Promise<AVMUTXOSet> {
...
export async function avmGetAllUTXOsForAddresses(
    ...
    if (len >= 1024) {
        let subUtxos = await avmGetAllUTXOsForAddresses(addrs, sourceChain, nextEndIndex)
...
export async function platformGetAllUTXOsForAddresses(
    ...
    if (len >= 1024) {
        let subUtxos = await platformGetAllUTXOsForAddresses(addrs, sourceChain, nextEndIndex)
        return utxoSet.merge(subUtxos)
    }
